...
function ContractPreview() {
  ...
  const generateContractHtml = useCallback((form) => {
    ...
    const infoBoxStyle = `
      <style>
        ...
        .dark .info-box, [data-theme='dark'] .info-box {
  ...
  useEffect(() => {
    // 미리보기 진입 시 무조건 라이트 모드로 (다크모드 클래스/스타일 완전 제거)
    document.documentElement.classList.remove('dark', 'dark-mode');
    document.body.classList.remove('dark', 'dark-mode');
    const appRoot = document.getElementById('root');
    if (appRoot) appRoot.classList.remove('dark', 'dark-mode');
    // App 컴포넌트에도 적용 (id/className이 있다면)
    const appDiv = document.querySelector('.App');
    if (appDiv) appDiv.classList.remove('dark', 'dark-mode');
